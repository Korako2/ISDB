...
CREATE TRIGGER check_speed_trigger
    ...
    ON vehicle_movement_history
    FOR EACH ROW
EXECUTE FUNCTION check_speed();
...
CREATE TRIGGER country_match_check_trigger
    BEFORE INSERT
    ON loading_unloading_agreement
    FOR EACH ROW
...
CREATE TRIGGER check_single_ownership_overlap_trigger
    BEFORE INSERT OR UPDATE
    ON vehicle_ownership
    FOR EACH ROW
EXECUTE FUNCTION check_single_ownership_overlap();
...
CREATE TRIGGER check_multiple_ownership_overlap
    ...
    ON vehicle_ownership
    FOR EACH ROW
EXECUTE FUNCTION check_multiple_ownership_overlap();
