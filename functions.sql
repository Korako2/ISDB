...
'
    DECLARE
        ...
        speed       float;
    BEGIN
        prev_record = (SELECT MILEAGE
                       FROM vehicle_movement_history
                       WHERE VEHICLE_ID = NEW.VEHICLE_ID
                       ORDER BY DATE DESC
                       LIMIT 1);
        prev_date = (SELECT DATE
                     FROM vehicle_movement_history
                     WHERE VEHICLE_ID = NEW.VEHICLE_ID
                     ORDER BY DATE DESC
                     LIMIT 1);
...
'
    ...
    BEGIN
        ...
        prev_pay_date = (SELECT DATE
                         ...
                         ORDER BY DATE DESC
                         LIMIT 1);
        prev_mileage = (SELECT MILEAGE
                        FROM vehicle_movement_history
                        WHERE VEHICLE_ID = var_vehicle_id
                          AND DATE <= prev_pay_date
                        ORDER BY DATE DESC
                        LIMIT 1);
        current_mileage = (SELECT MILEAGE
                           FROM vehicle_movement_history
                           WHERE VEHICLE_ID = var_vehicle_id
                             AND DATE >= prev_pay_date
                           ORDER BY DATE DESC
...
'
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1
                   FROM customer
                   WHERE person_id = v_person_id) THEN
...
'
    ...
    BEGIN
        ...
        FOR i IN 1..GREATEST(array_length(v_fuel_cards, 1), array_length(v_fuel_station_names, 1))
            LOOP
...
RETURNS TABLE (
    ...
    DECLARE
        ...
        vehicle_owner      INT   := -1; -- Переменная для проверки наличия владельца у автомобиля
        driver_ready       BOOLEAN; -- Переменная для проверки статуса водителя
    BEGIN
            ...
            IF vehicle_owner <> -1 THEN
                ...
                IF driver_ready THEN
                    ...
                    FROM (SELECT vmh.*,
                          ...
                          FROM vehicle_movement_history vmh
                          WHERE vmh.vehicle_id = current_vehicle.vehicle_id) vmh
                    WHERE vmh.rn = 1;
